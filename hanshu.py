...
def mubiaohanshu1(suoyin,D,topsuoyin):
    common = [x for x in suoyin if x in topsuoyin]
    f1=1-len(common)/D
    return f1


def mubiaohanshu2(suoyin, D,houxuanfinal):
    bizhi=[]
    for s in range(len(suoyin)):
        itemsuoyin=int(suoyin[s])
        rated=float(houxuanfinal[itemsuoyin][4])/93.5
...
def mubiaohanshu3(suoyin, D,houxuanfinal,sim):
    itemid = list()
    similarity = []
    ...
    f3 = 2 * sum(similarity) / (D * (D - 1))
    return f3


def mubiaohanshu4(suoyin, D,houxuanfinal):
    novelty = list()
